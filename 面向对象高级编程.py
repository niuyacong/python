...
#(Hello模块详见Reltion文件)
from relation.Hello import Hello  # 导入Hello类
h=Hello();
h.Hello(); #Hello,world
print(type(Hello)); # <class 'type'>
print(type(h));    # <class 'relation.Hello.Hello'>
# type()函数可以查看一个类型或变量的类型，Hello是一个class，它的类型就是type，而h是一个实例，它的类型就是class Hello。
#
# 我们说class的定义是运行时动态创建的，而创建class的方法就是使用type()函数。
#
# type()函数既可以返回一个对象的类型，又可以创建出新的类型，
# 比如，我们可以通过type()函数创建出Hello类，而无需通过class Hello(object)...的定义：(详见relation中Hello模块中nyc类)
# Hello模块通过type()定义的类，在本模块无法访问，可以在Hello模块使用
#
#
#
#
...
L2=list();
# L2.add(1);
# print(L2); # AttributeError: 'list' object has no attribute 'add'
