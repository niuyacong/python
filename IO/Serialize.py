...
def dict2student(sts):
    return  Student(sts['name'],sts['age'],sts['score'])

json_str= '{"age": 20, "score": 88, "name": "Bob"}'
stu=json.loads(json_str,object_hook=dict2student)
print(stu)
# <__main__.Student object at 0x00000000021D7550> 转换为类对象
print(stu.name);
# Bob
...
# ensure_ascii=False
# {"name": "小明", "age": 20}
# ensure_ascii为true（默认值），则保证在输出中输入的所有非ASCII字符都被转义。如果ensure_ascii为false，则这些字符按原样输出。
# 其他参数见文档：https://docs.python.org/3/library/json.html#json.dumps
